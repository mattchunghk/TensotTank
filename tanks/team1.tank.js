...
tank.loop(function (state, control) {
  ...
  if (state.radar.enemy) {
    ...
    // of the radar.
    var radarAngleDelta = Math.deg.normalize(
      enemyAngle - (state.radar.angle + state.angle)
    );
    ...
    var gunAngleDelta = Math.deg.normalize(
      enemyAngle - (state.gun.angle + state.angle)
    );
